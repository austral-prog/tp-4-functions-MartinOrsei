...
def total_letters(text):
    ...
    """
    cantidad_letras = count_vowels(text) + count_consonants(text)
...
def vowel_percentage(text):
    ...
    if not total_letters(text):
        return 0.0
    else:
        porcentaje_vocales = (count_vowels(text) * 100) / total_letters(text)
        return round(porcentaje_vocales, 1)
